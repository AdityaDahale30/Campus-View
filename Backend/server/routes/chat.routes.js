...
async function getUserByIdAndRole(userId, role) {
  ...
  if (normalizedRole === "hod") {
    const [rows] = await db.query(
    "SELECT id, name, 'hod' AS role, department, profile_image FROM hods WHERE id = ?",
      [userId]
    );
    return rows[0] || null;
  }
...
router.get("/allowed-users/:userId/:role", async (req, res) => {
  ...
  [rows] = await db.query(
   ...
   SELECT id, name, 'hod' AS role, department, profile_image FROM hods
    WHERE LOWER(department) = ?
    `,
  ...
  SELECT id, name, role, department, profile_image FROM hods
    WHERE LOWER(department) = ?
    `,
    [department, department]
  );
