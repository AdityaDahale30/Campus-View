...
const { date, status, time } = req.query; 
    ...
    if (role === "faculty" || role === "faculty_class_teacher") {
      query += ` AND (a.faculty_id = ? OR a.tg_id = ? OR a.class_teacher_id = ?)`;
  ...
  values.push(time);
    ...
    if (status === "unread") {
      if (role === "faculty" || role === "faculty_class_teacher") {
        ...
        query += ` AND a.tg_read = 0`;
      } else if (role === "hod") {
        query += ` AND a.hod_read = 0`;
      }
    }

    if (status === "read") {
      if (role === "faculty" || role === "faculty_class_teacher") {
        query += ` AND a.faculty_read = 1`;
      } else if (role === "faculty_teacher_guardian") {
        query += ` AND a.tg_read = 1`;
      } else if (role === "hod") {
        query += ` AND a.hod_read = 1`;
      }
    }
