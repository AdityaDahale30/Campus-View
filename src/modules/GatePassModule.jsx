...
function GatePassModule({ role, mode }) {
      ...
      <div key={req.id} className="gp-card-modern">
        ...
        {req.status === "Approved" && (
          <div className="gp-qr-box">
            <QRCodeCanvas
              value={`APPROVED GATE PASS
                     -
                     ${req.student_name.toUpperCase()}
                     Enrollment: ${req.enrollment_no}
                     Class: ${req.class_name?.replaceAll("_", " ")}
                     Department: ${req.department?.replaceAll("_", " ")}
                     Reason: ${req.reason}
                     Exit Time: ${req.exit_time?.slice(0, 5)}
                     Exit Date: ${new Date(req.exit_date).toLocaleDateString("en-GB")}
                     Approver Role: ${req.approver_role}
                     Status: ${req.status}`}
            />
          </div>
        )}
  ...
  const modalData =
    activeTab === "pending"
      ...
      : activeTab === "approved"
        ...
        : rejected;
  const approverName =
    studentApprover.active_approver_name || "No Approver Available"
  ...
  const approverRole =
    studentApprover.active_approver_role || "No Approver Available"
      ? "Teacher Guardian"
      : studentApprover.class_teacher_available === 1
        ? "Class Teacher"
        : "No Approver Available";
        ...
        {isFacultyApprover && (
          <div className="gatepass-card">
            ...
            <div className="availability-row">
              ...
              <span
                className={`status-text ${availability === 1
                  ? "available"
                  : availability === 0
                    ? "unavailable"
                    : ""
            ...
            <div className="availability-actions">
              <button
                type="button"
                className={`status-btn available-btn ${availability === 1 ? "active" : ""}`}
              ...
              <button
                type="button"
                className={`status-btn unavailable-btn ${availability === 0 ? "active" : ""}`}
