...
/* ========================================================== HELPERS ======================================================================== */
  ...
  const findLectureForSlot = (day, slot) => {
    const lectures = timetableData.filter((item) => item.day === day);

    let lecture = lectures.find((item) => item.time === slot);
  ...
  const shouldSkipCell = (day, slot) => {
    return timetableData.some((item) => {
      ...
      const mergedSlots = mergedMap[item.time];
      return mergedSlots && mergedSlots[1] === slot;
    });
  };
                ...
                {timeSlots.map((time) => {
                  ...
                  return (
                    <td key={time} colSpan={colSpan}>
                      {lecture && (
                        <div className={`lecture-card ${lecture.isEdited ? "edited-cell" : ""}`}>

                          {/* SUBJECT */}
                          <div className="subject">{lecture.subject}</div>

                          {/* TEACHER */}
                          <div className="teacher">{lecture.teacher}</div>
                          ...
                          {(role === "faculty_class_teacher" ||
                            ...
                            role === "hod") && (
                              <div className="action-group">
                                <button className="btn-taken" onClick={() => markLecture("taken", lecture)}>Taken</button>
                                <button className="btn-leave" onClick={() => markLecture("leave", lecture)}>Leave</button>
                                <button className="btn-off" onClick={() => markLecture("off", lecture)}>Off</button>
                              </div>
                            )}
                          ...
                          {(role !== "student") && (
                            <div className="action-group">
                              <button className="btn-edit" onClick={() => startEditing(lecture)}>Edit</button>
                            </div>
                          )}
                          ...
                          {lecture.isEdited === 1 && (
                            <div className="action-group">
                              <span className="edited-tag">Updated</span>
                              {(role !== "student") && (
                                <button className="btn-edit" onClick={() => undoLecture(lecture.id)}>Undo</button>
                              )}
                            </div>
                          )}
